...
main_map <- ggplot(data = world_simplified) +
  ...
  scale_fill_fermenter(
    breaks = pretty_breaks(n=8), # for 0t to 80t emissions
    palette = "YlOrRd",
    direction = 1
  ) +
  theme_minimal() +
  theme(
    ...
    legend.key.width = unit(0.6, "cm"), # Increase the width of the legend key
    legend.key.height = unit(0.5, "cm"), # Increase the height of the legend key
...
# mini_map_data <- st_crop(world_simplified, xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax)
  ...
  mini_map <- ggplot() +
    ...
    geom_sf(data = st_sfc(st_point(zoom_to), crs = 4326), size = 1) +
    ...
    geom_sf_text(
      data = filter(world_simplified, ranking == 1),
...
combined_plot <- cowplot::ggdraw(main_map) +
  cowplot::draw_plot(mini_map, x = 0.746, y = 0.255, width = 0.17, height = 0.17)
